...
// add by Carl, for KEY_SERIAL_NUMBER
...
public class Status extends SettingsPreferenceFragment implements Indexable {
    ...
    public void onCreate(Bundle icicle) {
        ...
        // modified by Carl, for KEY_SERIAL_NUMBER
        ...
        mQcRilHook = new QcRilHook(getPrefContext(), new QcRilHookCallback() {
            public void onQcRilHookReady() {
                int QCRIL_READ_NV = 0X80001;
                String serial = Build.SERIAL;
                AsyncResult ar = mQcRilHook.sendQcRilHookMsg(QCRIL_READ_NV, 2497);
                if (ar.exception == null) {
                    if (ar.result != null) {
                        serial = new String(((byte[]) ar.result));
                    }
                }
                if (!TextUtils.isEmpty(serial.trim())) {
                    mSerialNumberPreferenceController = new SerialNumberPreferenceController(getActivity(), serial);
                } else {
                    mSerialNumberPreferenceController = new SerialNumberPreferenceController(getActivity());
                }
                mSerialNumberPreferenceController.displayPreference(screen);
